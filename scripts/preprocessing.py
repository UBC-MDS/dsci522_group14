...
def main(df_path, path):
      df = pd.read_csv(load_data(df_path))
      ...
      ct = make_column_transformer(numerical_features, credit_feature, drop_features)

      with open(path+"/transformers/ct.pkl", 'wb') as pickle_file:
            pickle.dump(ct, pickle_file)

      with open(path+"/preprocessed/X_train.pkl", 'wb') as pickle_file:
            pickle.dump(X_train, pickle_file)

      with open(path+"/preprocessed/X_test.pkl", 'wb') as pickle_file:
            pickle.dump(X_test, pickle_file)

      with open(path+"/preprocessed/y_train.pkl", 'wb') as pickle_file:
            pickle.dump(y_train, pickle_file)

      with open(path+"/preprocessed/y_test.pkl", 'wb') as pickle_file:
      ...
      with open(path+"/preprocessed/transformed_X.pkl", 'wb') as pickle_file:
            pickle.dump(transformed_X_train, pickle_file)


def make_column_transformer(numerical_features, 
                            credit_feature, 
                            drop_features):
...
def load_data(df_path):
    with open(df_path, 'rb') as pickle_file:
      pickle_df = pickle.load(pickle_file)
      return StringIO(pickle_df)
